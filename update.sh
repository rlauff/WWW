...
deploy_static() {
  ...
  for d in candidates accepted rejected; do
    if [ -d "$target/$d" ]; then
      chmod a+x "$target/$d"                                  # enterable ...
      find "$target/$d" -type f -exec chmod a+r {} +          # ... files readable
    fi
  done
  # review.cgi records decisions in this log and cannot do anything else: it
  # has no write access to the pool directories. It runs as the web server's
  # user, so the log has to be writable by someone other than the owner.
  # Set LOG_MODE= if your host lets you use a shared group instead.
  local log="$target/review-log.jsonl"
  [ -e "$log" ] || : > "$log"
  chmod "${LOG_MODE:-666}" "$log"

  # Carry out the decisions recorded since the last deploy, then reindex from
  # what is actually on disk. This is what makes the manifests right after a
  # deploy: they are rebuilt from the directories rather than taken from the
  # commit, so a stale committed manifest cannot roll back a decision, and
  # candidates newly committed to the repo are picked up automatically.
  if [ -f "$target/rebuild_manifests.py" ]; then
    python3 "$target/rebuild_manifests.py" --root "$target" ||
      echo "  WARNING: rebuild_manifests.py reported problems (above)." >&2
  else
    echo "  WARNING: $target/rebuild_manifests.py is missing -- the manifests" \
         "are whatever the commit contained, which may be stale." >&2
  fi
